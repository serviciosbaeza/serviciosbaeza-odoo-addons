...
class PurchaseOrderConfirm(orm.TransientModel):
    ...
    def order_confirm(self, cr, uid, ids, context=None):
        ...
        orders = self.pool.get('purchase.order').read(
            cr, uid, context['active_ids'], ['state', 'name'], context=context)
        for record in orders:
            if not record['state'] == 'draft':
                raise orm.except_orm(
                    _('Warning!'),
                    _("Selected purchase order cannot be confirmed because it "
                      "isn't in 'Draft' state: %s." % record['name']))
