...
class SaleOrder(orm.Model):
    ...
    def action_button_generate_agreement_invoicing(
            self, cr, uid, ids, context=None):
        agreement_ids = []
        agreement_obj = self.pool['account.periodical_invoicing.agreement']
        agreement_line_obj = \
            self.pool['account.periodical_invoicing.agreement.line']
        ...
        if len(agreement_ids) == 1:
            ...
            view_ref = self.pool['ir.model.data'].get_object_reference(
                cr, uid, 'account_periodical_invoicing',
                'view_account_periodical_invocing_agreement_form')
